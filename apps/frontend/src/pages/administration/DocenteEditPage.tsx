...
let seccionCounter = 200;
const newSeccionId = () => `edit-${++seccionCounter}`;
...
export const DocenteEditPage = () => {
  ...
  const addSeccion = () =>
    setForm((p) =>
      p ? { ...p, secciones: [...p.secciones, { id: newSeccionId(), grado: '' }] } : p,
    );

  const updateSeccion = (id: string, grado: string) =>
    setForm((p) =>
      p ? { ...p, secciones: p.secciones.map((s) => (s.id === id ? { ...s, grado } : s)) } : p,
    );

  const removeSeccion = (id: string) =>
    setForm((p) => (p ? { ...p, secciones: p.secciones.filter((s) => s.id !== id) } : p));
